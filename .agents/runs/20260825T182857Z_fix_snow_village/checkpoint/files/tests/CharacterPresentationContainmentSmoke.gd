...
func _run() -> void:
	...
	for character in active_characters:
		...
		slot.set_character(character.sprite_frames)
		var portrait := slot.get_node("CardPanel/Portrait") as AnimatedSprite2D
		var half_size := CharacterPresentation.RUNTIME_CANVAS * CharacterPresentation.SLOT_SCALE * 0.5
	...
	var card_half := CharacterPresentation.RUNTIME_CANVAS * CharacterPresentation.CARD_SCALE * 0.5
