...
- Added three production head accessories after the user confirmed the generation budget: red pixel sunglasses (`head_sunglasses_red`), blue-water badge cowboy hat (`head_cowboy_hat`) and Vietnamese conical hat (`head_conical_hat_vietnam`). The three definitions are auto-discovered by `CosmeticRegistry`, purchasable/equippable through the existing head-accessory Shop/Inventory path, and synchronized through the existing room equipment snapshot.
...
- Placement is explicit and shared: sunglasses use `face`, hats use `hat`, and the four existing rings use `ring`. `AccessoryPresentation` supplies separate card/room/match-list/world anchors; only rings bob, preventing face/hat drift. `CosmeticPresentationSmoke` now requires all seven head assets and checks the new profiles.
- Focused verification after import: `COSMETIC_PRESENTATION PASS visible=14 compatible_total=20`, `ROOM_SLOT_LAYER_RESULT: pass`, `CHARACTER_PRESENTATION_RESULT: 20 passed | 0 failed`, and all `PlayableSmoke` checks pass. The dummy-renderer `LobbyV2Capture` limitation remains (no framebuffer for PNG capture); prior GPU tour evidence is still valid.
...
- Cosmetic presentation: 7 head accessories active across card/room/match-list/world (4 rings + face sunglasses + 2 hats), custom frame retired, legacy registry retained (`COSMETIC_PRESENTATION PASS visible=14 compatible_total=20`).
