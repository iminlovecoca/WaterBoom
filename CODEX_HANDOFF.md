...
## Accessory/network polish checkpoint — 2026-08-27

- `PlayerEquipmentService._commit` no longer sends a room-equipment RPC from an offline/stale local room. A host applies the snapshot locally; a client sends only while the server peer is live.
- `RoomManager._broadcast_room_players` now skips disconnected peers instead of issuing RPCs to stale IDs. Account ownership remains server-authoritative and the proposed equipment snapshot is sanitized against the server-owned cosmetics before broadcast.
- `NetworkManager.is_peer_connected(1)` now reports the connected client-to-server state as well as the host state, so client-side equipment updates are not silently skipped.
- Evidence: `tests/artifacts/accessory_ui_tour.avi` (18.17 s, 960×720/30 FPS) and the three layered UI captures cover lobby, Shop and Inventory accessory presentation.

## Head accessory art checkpoint — 2026-08-27

- Added five production head accessories after the user confirmed the generation budget: red pixel sunglasses (`head_sunglasses_red`), blue-water badge cowboy hat (`head_cowboy_hat`), Vietnamese conical hat (`head_conical_hat_vietnam`), birthday hat (`head_birthday_hat`) and royal crown (`head_crown_royal`). The definitions are auto-discovered by `CosmeticRegistry`, purchasable/equippable through the existing head-accessory Shop/Inventory path, and synchronized through the existing room equipment snapshot.
- The integrated image generator produced solid-blue staging images to make edge selection deterministic. A local connected-component chroma pass removed only border-connected staging pixels, preserving the blue/cyan interior accents of the glasses and hats. Final PNGs are RGBA, cropped to the non-transparent bbox with 16 px breathing room, and have alpha-zero corners; Godot `.import` files were generated by the editor reimport.
- Placement is explicit and shared: sunglasses use `face`, hats use `hat`, the four rings use `ring`, and the six player frames use a separate rounded layer behind the portrait. `AccessoryPresentation` supplies separate card/room/match-list/world anchors; only rings bob, preventing face/hat drift. `CosmeticPresentationSmoke` now requires all nine head assets and six frame definitions.
- Focused verification after import: `COSMETIC_PRESENTATION PASS visible=22 compatible_total=22`, `ROOM_SLOT_LAYER_RESULT: pass`, `CHARACTER_PRESENTATION_RESULT: 20 passed | 0 failed`, and all `PlayableSmoke` checks pass. The headless room capture intentionally skips framebuffer readback; prior GPU tour evidence remains the visual gate.

## Final QA checkpoint — 2026-08-27

- The user confirmed the generation budget for this turn. Five head-accessory assets were generated through the integrated `generate2dsprite`/`asset-gen` workflow, cleaned to transparent RGBA locally, imported by Godot and wired into the shop, inventory, room/lobby cards, match sidebar and gameplay anchors. Five player-frame definitions are also exposed through the new `KHUNG` category, with the default Aqua frame included in the six-definition total.
- Godot editor import: exit 0, with no temporary GDExtension warning file present.
- Full current smoke gate: `SMOKE_FAILED=0 TOTAL=19`, with `LocalAuthNetworkSmoke` exercised against a temporary local WebSocket host and the remaining scenes run headless.
- Focused gates: `ACCOUNT_DATABASE_RESULT: 22 passed | 0 failed`; `USER_BALLOON_SKINS PASS new=10 frames=4 icon=64`; `PLAYABLE_SMOKE_RESULT: 17 passed | 0 failed`; `TRANSPARENT_BALLOON_SKINS PASS new=10 alpha=clean canvas=128 icon=64`; `CHARACTER_PRESENTATION_RESULT: 20 passed | 0 failed`; `COSMETIC_PRESENTATION PASS visible=22 compatible_total=22`.
- Visual evidence: `tests/artifacts/accessory_ui_tour.avi` is 18.17 s at 960×720/30 FPS and covers Login → Lobby → Shop → Inventory → Match sidebar. The dummy-renderer PNG capture remains a Godot framebuffer limitation; it does not affect the GPU-backed tour or headless smoke gate.
...
- `scripts/cosmetics/AccessoryPresentation.gd`, `scripts/cosmetics/CosmeticRegistry.gd`, `scripts/ui/PlayerCardPreview.gd`, `scripts/player/PlayerVisual.gd`, `scripts/ui/MatchHUD.gd`, `scripts/core/BootManager.gd`: restore head-accessory presentation with shared ring/face/hat profiles, per-surface anchors, ring bob animation and safe visibility during bubble/terminal states; custom player frame remains retired without deleting compatibility data.
- `assets/cosmetics/head/sunglasses_red.png`, `assets/cosmetics/head/cowboy_hat.png`, `assets/cosmetics/head/conical_hat_vietnam.png` and `assets/cosmetics/head/generated_2026_08_27/`: three new transparent pixel head assets plus raw/chroma provenance.
- `resources/cosmetics/definitions/head_sunglasses_red.tres`, `head_cowboy_hat.tres`, `head_conical_hat_vietnam.tres`: shop metadata, placement profiles, and shared per-surface texture slots for the new assets.
...
- Cosmetic presentation: 9 head accessories active across card/room/match-list/world (4 rings + face sunglasses + 4 hats), plus 6 live player-frame definitions. The `KHUNG` category is visible in Shop/Inventory and PlayerCardPreview now uses the same cropped frame layer as lobby cards (`COSMETIC_PRESENTATION PASS visible=22 compatible_total=22`).
- GPU captures exit 0: Login, Room/Lobby v2, Shop, Inventory and Match sidebar. Headless room capture intentionally skips readback because Godot's dummy renderer exposes no readable ViewportTexture; the desktop/GPU capture remains the visual gate.
- GPU/runtime tour: `tests/artifacts/accessory_ui_tour.avi` dài 18.17 giây ở 960×720/30 FPS, đi qua Login → Lobby → Shop phụ kiện → Inventory → Match sidebar; các ảnh kiểm tra tương ứng nằm ở `tests/artifacts/lobby_v2_layered_cosmetics.png`, `shop_head_accessory_category.png` và `inventory_head_accessory_category.png`.
- The current review video is the AVI above; older MP4/contact-sheet names from the previous checkpoint are not part of the current artifact set.
- Account database smoke: 22/0, including atomic balloon purchase, ownership persistence and exact balance deduction.
- Map layout 90/0.
- Map gameplay 48/0.
...
- Godot editor import after the current head-accessory/economy pass: exit 0; no temporary GDExtension warning file remains, and the remaining addon DLL is the real template-debug binary.
...
Do not continue any of these without a separate scope decision:

- any additional paid bulk generation beyond the assets generated in this approved turn; the user has confirmed this turn's generation budget, but a future bulk pass still needs an explicit asset list;
...
- Dedicated V14 status/bubble art remains deferred; V11/V12 and older source bundles remain reversible fallbacks.

Review skill also requires user to choose exactly one: Bugbot or Security Review.

## Next safe sequence after approval

1. Playtest the active four-character roster in crowded 4v4 and boss scenes; make only per-character visual-scale/anchor corrections supported by capture evidence.
...
4. Continue low-resolution/device focus-navigation QA without changing the accepted Aqua Arcade v2 visual contract.
