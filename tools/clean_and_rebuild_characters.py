...
def extract_4_full_poses(image_path):
    ...
    # Accurate background detection: white/off-white and very light grey
    is_bg = (arr[:, :, 0] > 210) & (arr[:, :, 1] > 210) & (arr[:, :, 2] > 210)
    fg = ~is_bg
    
    # Remove isolated background noise
    ...
    for idx in range(1, num_features + 1):
        if sizes[idx] >= 10000:
            ...
            # Clean alpha fringe around borders
            alpha = sub_arr[:, :, 3]
            alpha[~sub_mask] = 0
...
for char_id, turnaround_fname in CHARACTERS:
    ...
    print(f"Cleaned and rebuilt {char_id}!")

print("All characters clean and uniform!")
