...
def main():
    ...
    for skin in skins:
        ...
        # Using a slightly more sensitive threshold to preserve white snowflakes/reflections
        _, alpha = cv2.threshold(mask, 10, 255, cv2.THRESH_BINARY)
        
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5,5))
        alpha = cv2.morphologyEx(alpha, cv2.MORPH_CLOSE, kernel)
        
        # Don't dilate or erode too much, just close holes.
        # We will use alpha directly to seed grabcut
        gc_mask = np.where(alpha > 128, 3, 2).astype('uint8')
        
        # DO NOT forcefully cut 15 pixels off the top! Just 2 pixels of absolute boundary.
        gc_mask[0:2, :] = 0
        gc_mask[-2:, :] = 0
        gc_mask[:, 0:2] = 0
        gc_mask[:, -2:] = 0
        ...
        # Filter to only the largest central component!
        final_alpha = get_central_component(final_alpha)
        
        # A tiny blur for anti-aliasing
